...
$pdf->SetAuthor('18 CNTN with DNR');
$pdf->SetTitle('18 CNTN with DNR');
$pdf->SetSubject('TcPdf');
$pdf->SetKeywords('PDF,18 CNTN with DNR');
...
$title_2 = table_cell1('  DIGITAL NONREPLICA', 34);
$title_3 = table_cell('  TOTAL CIRCULATION BY ISSUE');
$title_4 = table_cell('  RATE BASE');
$title_5 = table_cell1('  DIGITAL NONREPLICA', 34);
...
function table_cell_1($title, $size){
    $td = '
    <table width="'.$size.'%">
        <tr>
            <td style="color:#fff; background-color:#0244A1; font-size: bold;"><b>'.$title.'</b></td>
        </tr>
        <tr style="line-height: 30%;">
            <td></td>
        </tr>
...
$Digital_NonReplica1 = 
    '<table align="center" width="100%" border="1">
                <thead>
                    <tr>
				        <td style="background-color: #bcd4ee; line-height: 10px;" width="34%"><b><br><br>Total Analyzed <br> Nonpaid  Circulation</b></td>				        
                    </tr>
                </thead>    
...
if ($rbca) {
	...
	foreach ($rbca as $row) {
		$ratebasechange .= 
            '<tr>
                <td>' . $row['start_rb'] . ' through ' . $row['start_date'] . ', ' . $row['end_rb'] . ' starting ' . $row['end_date'] . '
                </td>
            </tr>';
	}
}
